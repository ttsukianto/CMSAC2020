...
subtitle: "Visualizing 1D categorical and continuous variables, plus scatter plots"
author: "Ron Yurko"
date: "06/04/2020"
---
...
## New dataset to explore - MVP Mike Trout's batted balls in 2019

Created dataset of batted balls by the American League MVP Mike Trout in 2019 season using [`baseballr`](http://billpetti.github.io/baseballr/)

```{r load-trout, warning = FALSE, message = FALSE}
library(tidyverse)
trout_batted_balls <- 
  read_csv("http://www.stat.cmu.edu/cmsac/sure/materials/data/xy_examples/trout_2019_batted_balls.csv")
head(trout_batted_balls)
```

--
- each row / observation is a batted ball from Trout's 2019 season
...
- `..count..` is an internal variable created by `stat_count()` called by `geom_bar()`...
...
We have the __population__ of Mike Trout's batted balls in the 2019 season
...
--
_What if we pretend this is a sample from all hypothetical Mike Trout 2019 seasons_?
...
.center[![](https://pbs.twimg.com/media/CdgyKHaWAAA815b.jpg)]
...
```{r exit-velo-summary}
summary(trout_batted_balls$exit_velocity)
sd(trout_batted_balls$exit_velocity)
```
...
- __Cons__:
  - Have to choose number of bins and bin locations (will revisit on Monday)
...
- Density estimation is the focus on Monday's lecture
